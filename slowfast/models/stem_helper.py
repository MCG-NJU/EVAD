...
class VideoModelStem(nn.Module):
    ...
    def __init__(
        ...
        self._construct_stem(dim_in, dim_out, norm_module, stem_func_name, use_keyway)

    def _construct_stem(self, dim_in, dim_out, norm_module, stem_func_name, use_keyway):
...
class X3DStem(nn.Module):
    ...
    def _construct_stem(self, dim_in, dim_out, norm_module):
        ...
        if self.use_keyway:
            assert self.kernel[0] > 1
            self.conv_key = nn.Conv3d(
                dim_out, dim_out, kernel_size=1, stride=1, padding=0, bias=False, groups=dim_out,
            )
    ...
    def forward(self, x):
        x = self.conv_xy(x)
        if self.use_keyway:
            num_frame = x.shape[2]  # prepare keyframe from Slow pathway
            assert num_frame % 2 == 0, "Only consider even case, check frames {}".format(num_frame)
            key_idx = num_frame // 2
            keyframe = x[:, :, key_idx:key_idx+1].clone()
        x = self.conv(x)
        if self.use_keyway:
            keyframe = self.conv_key(keyframe)
