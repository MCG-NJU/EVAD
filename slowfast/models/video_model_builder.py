...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        (dcn2, dcn3, dcn4, dcn5) = cfg.RESNET.DEFORM_ON_PER_STAGE
        ...
        if cfg.SLOWFAST.FUSION_TYPE == 'fair':
            _FUSE = FuseFastToSlow
        elif cfg.SLOWFAST.FUSION_TYPE == 'sjtu':
            _FUSE = SJTUFuse
        else:
            raise ValueError('Not support {}'.format(cfg.SLOWFAST.FUSION_TYPE))
...
class ResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.norm_module = get_norm(cfg)
        self.enable_detection = cfg.DETECTION.ENABLE
        self.use_fpn = cfg.MODEL.USE_FPN
        self.fpn_src = cfg.MODEL.FPN_SRC
        self.num_pathways = 1
        self._construct_network(cfg)
        init_helper.init_weights(
            self, cfg.MODEL.FC_INIT_STD, cfg.RESNET.ZERO_INIT_FINAL_BN
    ...
    def _construct_network(self, cfg):
        ...
        self.s5 = resnet_helper.ResStage(
            ...
            temp_kernel_sizes=temp_kernel[4],
            stride=cfg.RESNET.SPATIAL_STRIDES[3] if not self.use_fpn else [2],
            num_blocks=[d5],
            num_groups=[num_groups],
            num_block_temp_kernel=cfg.RESNET.NUM_BLOCK_TEMP_KERNEL[3],
            nonlocal_inds=cfg.NONLOCAL.LOCATION[3],
            ...
            stride_1x1=cfg.RESNET.STRIDE_1X1,
            inplace_relu=cfg.RESNET.INPLACE_RELU,
            dilation=cfg.RESNET.SPATIAL_DILATIONS[3] if not self.use_fpn else [1],
            norm_module=self.norm_module,
            deform=dcn5,
        )
        ...
        if self.enable_detection:
            ...
            self.head = HEAD(
                ...
                scale_factor=[cfg.DETECTION.SPATIAL_SCALE_FACTOR] if not self.use_fpn else [32],
                dropout_rate=cfg.MODEL.DROPOUT_RATE,
                act_func=cfg.MODEL.HEAD_ACT,
                aligned=cfg.DETECTION.ALIGNED,
    ...
    def pick_fpn_src(self, x, key_idx):
        if self.fpn_src == 'key':
            return x[0][:, :, key_idx]
        elif self.fpn_src == 'gap':
            B, C, T, H, W = x[0].shape
            return F.adaptive_avg_pool3d(x[0], (1, H, W)).squeeze(2)
        else:
            raise ValueError(self.fpn_src)
    ...
    def forward(self, x, bboxes=None):
        ...
        x = self.s2(x)
        if self.use_fpn:
            fpn_features.update({'res2': self.pick_fpn_src(x, key_idx)})
        for pathway in range(self.num_pathways):
            pool = getattr(self, "pathway{}_pool".format(pathway))
            x[pathway] = pool(x[pathway])
        x = self.s3(x)
        if self.use_fpn:
            fpn_features.update({'res3': self.pick_fpn_src(x, key_idx)})
        x = self.s4(x)
        if self.use_fpn:
            fpn_features.update({'res4': self.pick_fpn_src(x, key_idx)})
        x = self.s5(x)
        if self.use_fpn:
            fpn_features.update({'res5': self.pick_fpn_src(x, key_idx)})
            fpn_output = self.fpn_forward(fpn_features)
            fpn_features = [fpn_output['p{}'.format(f)] for f in range(2, 6)]
        else:
            fpn_features = None
        if self.enable_detection:
            x = self.head(x, bboxes, fpn_features)
        else:
            x = self.head(x)
        return x
