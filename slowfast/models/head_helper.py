...
from detectron2.modeling.poolers import ROIPooler, Boxes


class FPNRoIHead(nn.Module):
    """
    For Ablation studies: baseline + FPN
    """
    def __init__(self, dim_in, num_classes, pool_size, resolution, scale_factor, dropout_rate=0.0, act_func="softmax",
                 aligned=True, use_fpn=False):
        super(FPNRoIHead, self).__init__()
        assert len(pool_size) == len(dim_in) == 1, "only support SlowOnly"
        self.pooler = self._init_box_pooler()

        self.s0_pool = nn.MaxPool2d(7, stride=1)
    ...
    def _init_box_pooler():
        pooler_resolution = 7
        pooler_scales = [1.0 / x for x in [4, 8, 16, 32]]
        sampling_ratio = 2
    ...
    def forward(self, inputs, bboxes, fpn_features):
        assert len(inputs) == 1
        BATCH_SIZE = inputs[0].shape[0]
        fmt_boxes = self.convert_boxes(bboxes, BATCH_SIZE)
        out = self.pooler(fpn_features, fmt_boxes)
        x = self.s0_pool(out)
        x = x.view(x.shape[0], -1)
        x = self.projection(x)
        if self.training:
            return x   # using binary_cross_entropy_with_logits to make autocast happy
        return torch.sigmoid(x)

    def convert_boxes(self, boxes, N):
        idx = boxes[:, 0].long()
        fmt_boxes = [Boxes(boxes[idx == b][:, 1:]) for b in range(N)]
        return fmt_boxes
...
class ResNetRoIHead(nn.Module):
    ...
    def forward(self, inputs, bboxes, fpn_features=None):
        assert (
