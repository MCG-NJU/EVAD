...
from .deform_conv3d import ModulatedDeformConv3DWithOffset
...
class X3DTransform(nn.Module):
    ...
    def _construct(
        ...
        if self.use_keyway:
            assert self.temp_kernel_size > 1, self.temp_kernel_size
            bbb = nn.Conv3d(dim_inner, dim_inner, kernel_size=[1, 3, 3], stride=[1, str3x3, str3x3],
                            padding=[0, dilation, dilation], groups=num_groups, bias=False,
                            dilation=[1, dilation, dilation])
            bbb_bn = norm_module(num_features=dim_inner, eps=self._eps, momentum=self._bn_mmt)
            # ignore SE
            bbb_relu = Swish() if self._swish_inner else nn.ReLU(inplace=self._inplace_relu)
            self.keyway = nn.Sequential(bbb, bbb_bn, bbb_relu)
        else:
            self.keyway = None
    ...
    def forward(self, x):
        ...
        if self.use_keyway:
            keyframe = self.keyway(keyframe)
            x = torch.cat([x, keyframe], dim=2)
...
class BottleneckTransform(nn.Module):
    ...
    def _construct(
        ...
        if self.use_keyway:
            assert self.temp_kernel_size > 1, self.temp_kernel_size
            # build parallel way for keyframe
            aaa = nn.Conv3d(dim_in, dim_inner, kernel_size=1, stride=[1, str1x1, str1x1], padding=0, bias=False)
            aaa_bn = norm_module(num_features=dim_inner, eps=self._eps, momentum=self._bn_mmt)
            aaa_relu = nn.ReLU(inplace=self._inplace_relu)
            self.keyway = nn.Sequential(aaa, aaa_bn, aaa_relu)
        else:
            self.keyway = None

        if self.deform:
            self.b = ModulatedDeformConv3DWithOffset(
                dim_inner,
                dim_inner,
                3,  # [1, 3, 3],
                stride=str3x3,  # [1, str3x3, str3x3],
    ...
    def forward(self, x):
        ...
        # keyframe parallel way
        if self.use_keyway:
            keyframe = self.keyway(keyframe)
            x = torch.cat([x, keyframe], dim=2)  # append keyframe to the tail in Temporal dimension
    ...
    def _construct(
        ...
        for pathway in range(self.num_pathways):
            for i in range(self.num_blocks[pathway]):
                ...
                # use keyway or not
                use_keyway = use_keyway and pathway == 0 and self.temp_kernel_sizes[pathway][i] > 1
                # Construct the block.
                res_block = ResBlock(
