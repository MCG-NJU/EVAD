...
def pack_pathway_output(cfg, frames):
    ...
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_PATHWAY_ARCH:
        ...
        """ Fix the misalignment of the keyframe in Slow Pathway. e.g.:
        Previously, [0, 4, 8, 13, *17*, 22, 26, 31]
        Now,        [0, 4, 8, 12, *16*, 21, 26, 31]
        """
        num_fast = frames.shape[1]
        key_idx = num_fast // 2
        slow_idx_head = torch.linspace(0, key_idx, num_fast // cfg.SLOWFAST.ALPHA // 2 + 1)
        slow_idx_tail = torch.linspace(key_idx, num_fast - 1, num_fast // cfg.SLOWFAST.ALPHA // 2)
        slow_idx = torch.cat((slow_idx_head, slow_idx_tail[1:]), dim=0).long()
