...
def read_csv(csv_file, class_whitelist=None, load_score=False, total_lines=None):
...
def evaluate_ava_from_files(labelmap, groundtruth, detections, exclusions, total_lines=None):
    ...
    detections = read_csv(detections, class_whitelist, load_score=True, total_lines=total_lines)
...
def evaluate_ava(
    ...
    write_results(detections, "%s/detections_%s.csv" % (output_dir, name))
    write_results(groundtruth, "%s/groundtruth_%s.csv" % (output_dir, name))

    results = run_strict_evaluation(categories, groundtruth, detections, excluded_keys, strict_eval=strict_eval)

    logger.info("AVA eval done in %f seconds." % (time.time() - eval_start))
    return results


def run_strict_evaluation(categories, groundtruth, detections, excluded_keys, verbose=True, strict_eval=False,):
    if not strict_eval:
        ap50 = run_evaluation(categories, groundtruth, detections, excluded_keys, verbose_iou_idx=0)[0]

        results = dict(
            AP50=ap50["PascalBoxes_Precision/mAP@0.5IOU"]*100,
        )
        return results

    # array([0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95])
    iouThrs = np.linspace(.5, 0.95, int(np.round((0.95 - .5) / .05)) + 1, endpoint=True)
    ap = run_evaluation(categories, groundtruth, detections, excluded_keys, verbose_iou_idx=0, iouThrs=iouThrs)
    logger.info("!!!!!!!!!!!!!ap={}".format(ap[0]))

    APs = [ap[idx]["PascalBoxes_Precision/mAP@{}IOU".format(T)] for (idx, T) in enumerate(iouThrs)]
...
def run_evaluation(
    categories, groundtruth, detections, excluded_keys, verbose_iou_idx=None, iouThrs=None,
):
    ...
    for image_key in tqdm(boxes, total=len(boxes), desc="gt_box "):
    ...
    for image_key in tqdm(boxes, total=len(boxes), desc="pred_box "):
        if image_key in excluded_keys:
            logging.info(
                (
    ...
    if du.is_master_proc() and verbose_iou_idx is not None:
        pprint.pprint(metrics[verbose_iou_idx], indent=2)
    return metrics
...
def get_ava_eval_data(
    ...
    count = 0
    for i in tqdm(range(scores.shape[0]), desc="Scores "):
