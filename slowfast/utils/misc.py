...
from .distributed import init_dist_slurm
...
def launch_job(cfg, init_method, func, daemon=False, local_rank=-1):
    """
    ...
    """
    if cfg.NUM_GPUS > 1:
        msg = "NUM_GPUS should be divisible by NUM_SHARDS, {} vs {}".format(cfg.NUM_GPUS, cfg.NUM_SHARDS)
        assert cfg.NUM_GPUS % cfg.NUM_SHARDS == 0, msg
        num_gpus_per_machine = cfg.NUM_GPUS // cfg.NUM_SHARDS
...
def launch_job_slurm(cfg, init_method, func, daemon=False, local_rank=-1):
    if cfg.NUM_GPUS > 1:
        init_dist_slurm(cfg.DIST_BACKEND, port=cfg.PORT)
        ret = func(cfg)
    else:
        func(cfg=cfg)


def launch_job_tencent(cfg, init_method, func, daemon=False, local_rank=-1):
    if cfg.NUM_GPUS > 1:
