...
from .cocoeval import create_small_table
...
class AVAMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        self.loss_ce = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_bce = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_bbox = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_giou = ScalarMeter(cfg.LOG_PERIOD)
        ...
        self.excluded_keys = read_exclusions(
            os.path.join(cfg.DATA.PATH_TO_DATA_DIR, cfg.AVA.ANNOTATION_DIR, cfg.AVA.EXCLUSION_FILE)
        )
        self.categories, self.class_whitelist = read_labelmap(
            os.path.join(cfg.DATA.PATH_TO_DATA_DIR, cfg.AVA.ANNOTATION_DIR, cfg.AVA.LABEL_MAP_FILE)
        )
        gt_filename = os.path.join(
            cfg.DATA.PATH_TO_DATA_DIR, cfg.AVA.ANNOTATION_DIR, cfg.AVA.GROUNDTRUTH_FILE
        )
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if self.mode == "train":
            stats = {
                ...
                "loss_ce": self.loss_ce.get_win_median(),
                "loss_bce": self.loss_bce.get_win_median(),
                "loss_bbox": self.loss_bbox.get_win_median(),
                "loss_giou": self.loss_giou.get_win_median(),
                "lr": self.lr,
    ...
    def reset(self):
        ...
        self.loss_ce.reset()
        self.loss_bce.reset()
        self.loss_bbox.reset()
        self.loss_giou.reset()

        self.all_preds = []
        self.all_ori_boxes = []
        self.all_metadata = []

    def update_stats(self, preds, ori_boxes, metadata, loss=None, lr=None, loss_dict=None):
        """
        ...
        Args:
            ...
            loss_dict (dict): all kinds of loss.
        """
        ...
        if loss_dict is not None:
            self.loss_ce.add_value(loss_dict['loss_ce'])
            self.loss_bce.add_value(loss_dict['loss_bce'])
            self.loss_bbox.add_value(loss_dict['loss_bbox'])
            self.loss_giou.add_value(loss_dict['loss_giou'])
        if lr is not None:
            self.lr = lr

    def finalize_metrics(self, log=True, skip_save=False):
        """
        ...
        if not skip_save:
            ...
            save_all = dict(
                preds=all_preds,
                ori_boxes=all_ori_boxes,
                metadata=all_metadata
            )
            if self.cfg.TEST.SAVE2TENSOR:
                torch.save(save_all, "{}/save2tensor.pt".format(self.cfg.OUTPUT_DIR))
        else:
            save_all = torch.load(self.cfg.TEST.TENSOR_PATH)
            all_preds = save_all['preds']
            all_ori_boxes = save_all['ori_boxes']
            all_metadata = save_all['metadata']
        ...
        self.full_map = evaluate_ava(
            ...
            strict_eval=self.cfg.AVA.STRICT_EVAL,
        )
        if log:
            stats = {"mode": self.mode}
            stats.update(self.full_map)
            logging.log_json_stats(stats)
        ...
        if self.mode in ["val", "test"]:
            ...
            if du.is_master_proc():
                with open("{}/ap_summary.txt".format(self.cfg.OUTPUT_DIR), 'a') as fh:
                    fh.write("============= mode: {}   cur_epoch {}  (%)=============\n".format(self.mode, cur_epoch + 1))  # noqa
                    fh.write("AVA mAP:\n")
                    table_res = create_small_table(self.full_map)
                    fh.write("{}\n\n".format(table_res))
