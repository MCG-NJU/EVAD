...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    """
        all_iou_results = self._evaluation.evaluate()
        all_iou_metrics = []
        for one_iou_thres, one_result in zip(self.iouThrs, all_iou_results):
            all_iou_metrics.append(self.evaluate_per_iou(one_iou_thres, one_result))
        return all_iou_metrics

    def evaluate_per_iou(self, iou_threshold, res):
        (
            per_class_ap,
            mean_ap,
            _,
...
class PascalDetectionEvaluator(ObjectDetectionEvaluator):
    ...
    def __init__(self, categories, iouThrs):
...
class ObjectDetectionEvaluation(object):
    ...
    def _initialize_detections(self):
        ...
        scores_per_class = [[] for _ in range(self.num_class)]
        self.scores_per_class_per_iou = [deepcopy(scores_per_class) for _ in range(len(self.iouThrs))]
        tp_fp_labels_per_class = [[] for _ in range(self.num_class)]
        self.tp_fp_labels_per_class_per_iou = [deepcopy(tp_fp_labels_per_class) for _ in range(len(self.iouThrs))]
        num_images_correctly_detected_per_class = np.zeros(self.num_class)
        self.num_images_correctly_detected_per_class_per_iou = [deepcopy(num_images_correctly_detected_per_class) for _ in range(len(self.iouThrs))]
        average_precision_per_class = np.empty(self.num_class, dtype=float)
        average_precision_per_class.fill(np.nan)
        self.average_precision_per_class_per_iou = [deepcopy(average_precision_per_class) for _ in range(len(self.iouThrs))]
        precisions_per_class = []
        self.precisions_per_class_per_iou = [deepcopy(precisions_per_class) for _ in range(len(self.iouThrs))]
        recalls_per_class = []
        self.recalls_per_class_per_iou = [deepcopy(recalls_per_class) for _ in range(len(self.iouThrs))]
        corloc_per_class = np.ones(self.num_class, dtype=float)
        self.corloc_per_class = [deepcopy(corloc_per_class) for _ in range(len(self.iouThrs))]
    ...
    """
        ...
        for iou_idx in range(len(self.iouThrs)):
            for i in range(self.num_class):
                """Now, `scores` is List[ori `scores`]"""
                if scores[iou_idx][i].shape[0] > 0:
                    self.scores_per_class_per_iou[iou_idx][i].append(scores[iou_idx][i])
                    self.tp_fp_labels_per_class_per_iou[iou_idx][i].append(tp_fp_labels[iou_idx][i])
    ...
    """
        ...
        for iou_idx in tqdm(range(len(self.iouThrs)), desc="Threshold"):
            scores_per_class = self.scores_per_class_per_iou[iou_idx]
            tp_fp_labels_per_class = self.tp_fp_labels_per_class_per_iou[iou_idx]
            for class_index in tqdm(range(self.num_class), desc="Classes ", disable=len(self.iouThrs)>1):
                if self.num_gt_instances_per_class[class_index] == 0:
                    continue
                if not scores_per_class[class_index]:
                    scores = np.array([], dtype=float)
                    tp_fp_labels = np.array([], dtype=bool)
                else:
                    scores = np.concatenate(scores_per_class[class_index])
                    tp_fp_labels = np.concatenate(
                        tp_fp_labels_per_class[class_index]
                    )
                if self.use_weighted_mean_ap:
                    raise NotImplementedError
                ...
                self.precisions_per_class_per_iou[iou_idx].append(precision)
                self.recalls_per_class_per_iou[iou_idx].append(recall)
                average_precision = metrics.compute_average_precision(
                    precision, recall
                )
                self.average_precision_per_class_per_iou[iou_idx][class_index] = average_precision

        self.corloc_per_class_per_iou = [metrics.compute_cor_loc(
            self.num_gt_imgs_per_class,
            self.num_images_correctly_detected_per_class_per_iou[i],
        ...
        if self.use_weighted_mean_ap:
            ...
            mean_ap_per_iou = []
            for i in range(len(self.iouThrs)):
                mean_ap = np.nanmean(self.average_precision_per_class_per_iou[i])
                mean_ap_per_iou.append(mean_ap)

        mean_corloc_per_iou = [np.nanmean(self.corloc_per_class_per_iou[i]) for i in range(len(self.iouThrs))]
        return [ObjectDetectionEvalMetrics(
            self.average_precision_per_class_per_iou[idx],
            mean_ap_per_iou[idx],
            self.precisions_per_class_per_iou[idx],
            self.recalls_per_class_per_iou[idx],
            self.corloc_per_class_per_iou[idx],
            mean_corloc_per_iou[idx],
