...
class PerImageEvaluation(object):
    ...
    def __init__(self, num_groundtruth_classes, iouThrs):
    ...
    """
        ...
        result_scores = [[] for _ in range(len(self.iouThrs))]
        result_tp_fp_labels = [[] for _ in range(len(self.iouThrs))]
        for i in range(self.num_groundtruth_classes):
            ...
            scores_and_tpfplabels = self._compute_tp_fp_for_single_class(
            ...
            for idx, (scores, tp_fp_labels) in enumerate(scores_and_tpfplabels):
                result_scores[idx].append(scores)
                result_tp_fp_labels[idx].append(tp_fp_labels)
    ...
    """
        if detected_boxes.size == 0:
            return [(np.array([], dtype=float), np.array([], dtype=bool)) for _ in range(len(self.iouThrs))]
        ...
        if groundtruth_boxes.size == 0:
            return [(scores, np.zeros(num_detected_boxes, dtype=bool)) for _ in range(len(self.iouThrs))]

        all_results = []
        for one_threshold in self.iouThrs:
            one_res = self._cell_iou(one_threshold, num_detected_boxes, iou,
                                     copy.deepcopy(scores),
                                     copy.deepcopy(groundtruth_is_difficult_list),
                                     copy.deepcopy(groundtruth_is_group_of_list))
            all_results.append(one_res)
        return all_results

    def _cell_iou(
        self,
        one_matching_threshold,
        num_detected_boxes,
        iou,
        scores,
        groundtruth_is_difficult_list,
        ...
        """ It is part of the original `_compute_tp_fp_for_single_class`. Now, it used to calculate one iou threshold

        sililar parameters as `_compute_tp_fp_for_single_class` with one addition: iouThres
        ...
        if iou.shape[1] > 0:
            ...
            for i in range(num_detected_boxes):
                ...
                if iou[i, gt_id] >= one_matching_threshold:
