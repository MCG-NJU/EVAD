...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, amp=None):
    """
    Save a checkpoint.
    Args:
    ...
    if convert_from_caffe2:
        ...
        for model_key in ['model', 'module']:
            if model_key in checkpoint:
                checkpoint["model_state"] = checkpoint[model_key]
            ...
            if clear_name_pattern:
                for item in clear_name_pattern:
                    ...
                    for k in checkpoint["model_state"]:
                        ...
                        elif k.startswith('decoder.') or k.startswith('encoder_to_decoder.'):
                            k_re = 'det_head.' + k
...
def load_train_checkpoint(cfg, model, optimizer, amp=None):
    """
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, cfg.NUM_GPUS > 1, optimizer, amp
        )
