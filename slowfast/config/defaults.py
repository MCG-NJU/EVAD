...
_C.EVAL_ONLY = False
_C.LAUNCHER = 'pytorch'
_C.PORT = 29660  # used by slurm port
_C.ADDR = ''
# ---------------------------------------------------------------------------- #
# Batch norm options
# ---------------------------------------------------------------------------- #
...
# test every epoch after
_C.TRAIN.TEST_AFTER_EPOCH = 5
...
_C.MODEL.SINGLE_PATHWAY_ARCH = ["c2d", "i3d", "slow", "x3d", "swin", "vit"]
...
_C.ViT.DROP_PATH_RATE = 0.
_C.ViT.LAYER_DECAY = 1.0
_C.ViT.DEPTH = 12
_C.ViT.NUM_HEADS = 12
_C.ViT.MLP_RATIO = 4
...
# Head learning rate is multiplied by LR_SCALE.
...
# Total Number of GPUs on all machine to use (applies to both training and testing).
_C.NUM_GPUS = 1
...
# **************** below only used by SparseDatasetMapper() **************************
_C.AVA.CROP_ENABLED = True
_C.AVA.CROP_SIZE = 224

# copy-paste from SparseRCNN
_C.AVA.MIN_SIZE_TRAIN = [480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800]
_C.AVA.MAX_SIZE_TRAIN = 1333
_C.AVA.MIN_SIZE_TRAIN_SAMPLING = "choice"

_C.AVA.MIN_SIZE_TEST = 800
_C.AVA.MAX_SIZE_TEST = 1333

_C.AVA.STRICT_EVAL = True
# **************** above only used by SparseDatasetMapper() **************************
...
# Backend to process image, includes `pytorch` and `cv2`, `detectron2`.
