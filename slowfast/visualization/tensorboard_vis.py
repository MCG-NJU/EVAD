...
class TensorboardWriter(object):
    ...
    def add_histograms(self, data_dict, global_step=None):
        """
        ...
        :return:
        """
        step = global_step if global_step is not None else self.global_step
        if self.writer is not None:
            for key, item in data_dict.items():
                self.writer.add_histogram(key, item, step)

    def update_global_step(self, v):
        self.global_step = v
