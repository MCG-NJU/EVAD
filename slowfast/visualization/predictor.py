...
class E2E_Predictor:
    """
    End-to-End Action Detection, without Detectron2 person detector
    """
    ...
    def __call__(self, task):
        ...
        frames, bboxes = task.frames, task.bboxes
        if bboxes is not None:
            raise NotImplementedError("Hi, bboxes should have been None now")
            bboxes = cv2_transform.scale_boxes(
                self.cfg.DATA.TEST_CROP_SIZE,
                bboxes,
                task.img_height,
        ...
        ori_preds = self.model(inputs)

        if self.cfg.NUM_GPUS:
            # preds = preds.cpu()
            preds = ori_preds['labels'].cpu()
            bboxes = ori_preds['boxes'].detach().cpu()


        preds = preds.detach()
        task.add_action_preds(preds[0][None, :])

        box_coor = [
            torch.cat(
                [torch.full((bboxes[i].shape[0], 1), float(i),
                            device=bboxes.device), bboxes[i]], dim=1
            )
            for i in range(len(bboxes))
        ]
        bboxes = torch.cat(box_coor, dim=0)[:, 1:]
        bboxes[:, 0].clamp_min_(0)
        bboxes[:, 1].clamp_min_(0)
        bboxes[:, 2].clamp_max_(320-1)
        bboxes[:, 3].clamp_max_(256-1)

        task.add_bboxes(bboxes[0][None, :])
...
class ActionPredictor:
    ...
    def __init__(self, cfg, async_vis=None, gpu_id=None, end2end=False):
        """
        Args:
            cfg (CfgNode): configs. Details can be found in
                slowfast/config/defaults.py
        ...
        """
        if end2end:
            self.predictor = E2E_Predictor(cfg=cfg, gpu_id=gpu_id)
        else:
