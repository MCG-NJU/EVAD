...
class SetCriterion(nn.Module):
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        if self.use_focal:
            ...
            # only a single class "person" for person detector CE loss
    ...
    def action_cls_loss(self, action, target):
        if not self.cfg.MODEL.SparseRCNN.SOFTMAX_POSE:
            return F.binary_cross_entropy_with_logits(action, target)  # remove Sigmoid in header
        else:
            pose_pred = action[:, :14]  # first 14 is pose label
            pose_tgt = target[:, :14].argmax(dim=1)
            pose_loss = F.cross_entropy(pose_pred, pose_tgt)
            other_act_loss = F.binary_cross_entropy_with_logits(action[:, 14:], target[:, 14:])
            return pose_loss + other_act_loss
    ...
    def forward(self, outputs, targets, writer=None):
        ...
        indices = self.matcher(outputs_without_aux, targets, writer=writer)
        ...
        return losses, last_layer_indices, self._get_src_permutation_idx(last_layer_indices)
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, writer=None):
        ...
        if writer is not None:
            writer.add_histograms(dict(
                cost_bbox=cost_bbox, cost_class=cost_class, cost_giou=cost_giou
            ))
